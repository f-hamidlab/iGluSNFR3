...
function event_cluster = cluster_events(ROI,event_cluster,n)
    ...
    if ROI(n).Area == 1  % only one pixel
        ...
        event_cluster(end).dff = reshape(signal_dfof(ROI(n).t,idx),1,[]);
    ...
    elseif isscalar(peak_x) % only one event
        ...
        [~, ~, cluster_dfof, ~, ~] = signal_processing(sum(signal_raw(:, idx),2), ops, ops.experiment_type);
    ...
    elseif ~isempty(peak_x) % more than one event and more than one pixel
        ...
        for i = 1:max(T)
            ...
            x = max([1,x_weighted-r]):min([x_weighted+r, ops.Nx]);
            y = max([1,y_weighted-r]):min([y_weighted+r, ops.Ny]);
            [X,Y] = meshgrid(x,y);
            ...
            if max(T) == 1
                % calculate signal for all pixels in the cluster
                [~, cluster_df, cluster_dfof, cluster_dfof_movemean, cluster_edge] = signal_processing(sum(signal_raw(:, idx),2), ops, ops.experiment_type);
                
                event_cluster(end).dfof = cluster_dfof;
                px = struct('idx', 1, 'df', [], 'dff_t', [], 'dff', [], 'ipt', [], 'dff_from_ipt', []); % define empty structure
                ...
                % calculate signal for all pixels in the cluster
                [~, ~, cluster_dfof, ~, ~] = signal_processing(sum(signal_raw(:, px_centre),2), ops, ops.experiment_type);
                event_cluster(end).dfof = cluster_dfof;
